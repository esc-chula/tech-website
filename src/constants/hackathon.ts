// REGISTARTION
export const HACKATHON_TICKET_EXPIRY_DAYS = 2
export const HACKATHON_MAX_TEAMS = 10

// LINKS
export const HACKATHON_IG_URL =
  'https://intania.link/hack-ig?utm_source=web&utm_medium=corner+button'
export const HACKATHON_OPENCHAT_URL =
  'https://intania.link/hack-openchat?utm_source=web&utm_medium=corner+button'

// GAME: JACKPOT
export const HACKATHON_GAME_JACKPOT_MODE: 'FULL_TICKET' | 'PER_CHARACTER' =
  'FULL_TICKET'
export const HACKATHON_GAME_JACKPOT_FULL_TICKET_MODE_RATE = 0.015
export const HACKATHON_GAME_JACKPOT_PER_CHARACTER_MODE_RATE = 0.15
export const HACKATHON_GAME_JACKPOT_SYMBOLS = [
  '[8]',
  '<$>',
  '[B]',
  '(#)',
  '{*}',
  '[X]',
]
export const HACKATHON_GAME_JACKPOT_TICKET_CODES = [
  'U7MF00AI87',
  '7N7OOGN4AY',
  'R2WEQSTN8D',
  '7V2Q5N7VDJ',
]
export const HACKATHON_GAME_JACKPOT_PROGRESS_STORAGE_SECRET_KEY =
  'hackathon2025'
