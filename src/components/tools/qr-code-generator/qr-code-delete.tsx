...
const DeleteQRCode: React.FC<{
  name: string;
  onDelete: () => void;
  onCancel: () => void;
}> = ({ name, onDelete, onCancel }) => {
  ...
  return (
    ...
    >
      <p className="w-full font-bold text-base text-center text-white md:text-xl lg:text-2xl">
        Delete {name} ?
      </p>
      <div className="flex flex-col gap-2">
        <div className="flex flex-row justify-between items-end">
          <div className="font-semibold text-sm text-white md:text-base lg:text-lg grow">
            Type {name} to confirm :
          </div>
          {isDeleteError ? (
            <div className="w-fit text-red-500 text-xs md:text-sm">
              Please Try Again
            </div>
          ) : null}
        </div>
        <input
          ...
          className={cn(
            ...
            {
              'border-neutral-600': !isDeleteError,
              'border-red-600': isDeleteError,
            },
          )}
