...
const QRCodePage: React.FC = () => {
  ...
  useEffect(() => {
    if (showCreateQrCode) document.body.style.overflow = 'hidden';
  ...
  const handleCreateQrCode = (data: QrCode): void => {
      ...
      <div className="flex flex-col justify-start gap-20 w-full h-full text-secondary">
        ...
        <div className="flex flex-col gap-8">
          ...
          >
            ...
            <div className="ml-1 font-semibold text-lg md:text-xl xl:text-2xl">
              Back
            </div>
          </button>
          <h1 className="font-bold text-4xl text-center md:text-5xl uppercase leading-normal">
            QR CODE GENERATOR
          </h1>
        </div>
        {/* Qr-Code */}
        <div className="flex flex-col justify-start items-center gap-16 w-full">
          ...
          <div className="gap-6 sm:gap-14 grid grid-cols-[repeat(auto-fit,_minmax(320px,1fr))] sm:grid-cols-[repeat(auto-fit,_minmax(400px,1fr))] auto-rows-[minmax(120px,_auto)] w-full">
            ...
            {mockData.map((data) => (
              <div
                key={data.name}
                className="justify-items-center grid col-span-1 w-full h-full"
              >
                <QRCodeItem
                  editAt={data.editAt}
                  name={data.name}
